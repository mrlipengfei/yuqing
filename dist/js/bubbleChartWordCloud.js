...
(function () {
            ...
            // 主函数
            var main = function main( /* value */v) {
                        // 渲染（值）
                        render_value(v);
                        // 渲染（元素）
                        render_element(v);
            ...
            // 渲染（值）
            var render_value = function render_value( /* value */v) {
                        var $, a, b, c, i;
                        ...
                        // 数据
                        if ($ = v['data']) for (value['data'] = $, a = value['option']['series'][0]['nodes'], i = 0; b = $[i]; i++) {
                                    a.push({
                                                ...
                                                // 颜色
                                                itemStyle: { normal: { color: b.color || 'rgba(' + Math.round(Math.random() * 255) + ',' + Math.round(Math.random() * 255) + ',' + Math.round(Math.random() * 255) + ',1)', opacity: '.8' } }
            ...
            // 渲染（元素）
            var render_element = function render_element( /* value */v) {
            ...
            // 处理程序（点击）
            var handler_click = function handler_click( /* event */e) {
